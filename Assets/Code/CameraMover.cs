...
{
    private TouchDetection front,back,left,right;
    // WASD：前後左右の移動
    // QE：上昇・降下
    // 右ドラッグ：カメラの回転
    // 左ドラッグ：前後左右の移動
    ...
    void Start ()
    {
        front=GameObject.Find("Front").GetComponent<TouchDetection>();
        back=GameObject.Find("Back").GetComponent<TouchDetection>();
        right=GameObject.Find("Right").GetComponent<TouchDetection>();
        left=GameObject.Find("Left").GetComponent<TouchDetection>();
...
*/
        ...
        if (right.tou==1) { campos += _camTransform.right * Time.deltaTime * _positionStep; }
        if (Input.GetKey(KeyCode.A)) { campos -= _camTransform.right * Time.deltaTime * _positionStep; }
        if (left.tou==1) { campos -= _camTransform.right * Time.deltaTime * _positionStep; }
        ...
        if (front.tou==1) { campos += _camTransform.forward * Time.deltaTime * _positionStep; }
        if (Input.GetKey(KeyCode.S)) { campos -= _camTransform.forward * Time.deltaTime * _positionStep; }
        if (back.tou==1) { campos -= _camTransform.forward * Time.deltaTime * _positionStep; }
